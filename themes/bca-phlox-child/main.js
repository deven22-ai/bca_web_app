...
function scrollMargin() {
  document.querySelectorAll('a[href^="#"]').forEach((anchor) => {
    anchor.addEventListener("click", function (e) {
      ...
      const headerHeight =
        document.querySelector("header")?.offsetHeight || 100; // Get the header height
  ...
  document.querySelectorAll(".bca-faq__question").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      document.querySelectorAll(".bca-faq__item").forEach((i) => {
        ...
        i.querySelector(".bca-faq__question").setAttribute(
          "aria-expanded",
          "false",
        );
      });
...
const formSubmitBtn = document.querySelector(
  "form .bca-form-submit input[type=submit]",
);
